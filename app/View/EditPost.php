...
<form id="edit-post" method="post" action="/posts/<?= $post->getId() ?>/edit" class="contents">
    <article class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md">
        ...
        <div class="mt-2 flex flex-col gap-4">
            <input type="text" name="title" value="<?= $post->getTitle() ?>" class="border border-slate-300 rounded-lg p-2"
                   required>
            <textarea name="content" form="edit-post" placeholder="Content"
                      class="border border-slate-300 rounded-lg p-2" required><?= $post->getContent() ?></textarea>
